...
async function main() {
  try {
    ...
    // Get the MCP server path after setup is complete
    ...
    log(`\n${colors.bright}Don't forget to add the chrome extension:${colors.reset}`);
    log(`${colors.cyan}• Extension Directory:${colors.reset} ${join(rootDir, 'chrome-extension')}`);
    log(`${colors.cyan}• toggle on "Developer mode"`);
    log(`${colors.cyan}• Load in Chrome:${colors.reset} chrome://extensions/ → Load unpacked`);
